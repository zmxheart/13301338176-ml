...
This sample demonstrates the use of device enqueue for searching for a key using binary search.
...
Run the sample
	...
	Execute the sample typing the following command 
		Windows : $> BinarySearchDeviceSideEnqueue.exe 	[location: <sample_name>\bin\x86_64\<build> ]
		Linux :	  $> ./BinarySearchDeviceSideEnqueue	[location: <sample_name>/bin/x86_64/<build> ]
	...
	-k,   --nKeys                                           number of input keys
	-eq,  --devEnqueue                                      device-side kernel enqueue : To run the sample using openCL 1.2 choose <eq 0> 
	-rs,  --randSeed                                        <value> : To generate different sets of random values
